...
WebSocketGateway::WebSocketGateway(
    ...
    , start_time_ms_(duration_cast<milliseconds>(
...
bool WebSocketGateway::start() {
    ...
    try {
        ...
        std::cout << "[WebSocketGateway] Started on " << config_.host 
                  << ":" << config_.port 
                  << " with " << config_.broadcast_fps << " FPS" << std::endl;
        return true;
    } catch (const std::exception& e) {
        std::cerr << "[WebSocketGateway] Failed to start: " << e.what() << std::endl;
...
void WebSocketGateway::stop() {
    ...
    // Stop uWebSockets app
    if (app_) {
        // uWebSockets doesn't have a direct stop method in this version
        // We'll rely on the thread termination
    }
...
void WebSocketGateway::server_thread_func() {
    try {
        ...
        app_->ws<PerSocketData>("/*", {
            .compression = config_.enable_compression ? uWS::SHARED_COMPRESSOR : uWS::DISABLED,
        ...
        if (enable_http_endpoints_) {
            app_->get("/health", [](uWS::HttpResponse<false>* res, uWS::HttpRequest* req) {
                ...
                res->end(R"({"status":"healthy","service":"matching-engine-websocket"})");
            });
            ...
            app_->get("/metrics", [this](uWS::HttpResponse<false>* res, uWS::HttpRequest* req) {
                ...
                ss << R"({
  "uptime_ms": )" << stats.uptime_ms << R"(,
  "connections_active": )" << stats.connections_active << R"(,
  "connections_total": )" << stats.connections_total << R"(,
  "messages_sent": )" << stats.messages_sent << R"(,
  "bytes_sent": )" << stats.bytes_sent << R"(,
  "queue_depth": )" << stats.queue_depth << R"(,
  "messages_per_second": )" << stats.messages_per_second << R"(
})";
                res->writeStatus("200 OK");
                res->writeHeader("Content-Type", "application/json");
                res->end(ss.str());
        ...
        app_->listen(config_.host, config_.port, [this](auto* listenSocket) {
            if (listenSocket) {
                ...
                std::cerr << "[WebSocketGateway] Failed to listen on " 
        ...
        std::cerr << "[WebSocketGateway] Server thread error: " << e.what() << std::endl;
        running_.store(false);
    }
}
...
void WebSocketGateway::on_disconnection(uWS::WebSocket<false, true>* ws, int code, std::string_view message) {
    connections_.remove(ws);
    std::cout << "[WebSocketGateway] Connection closed, remaining: " 
              << connections_.size() << std::endl;
}

void WebSocketGateway::on_message(uWS::WebSocket<false, true>* ws, std::string_view message, uWS::OpCode opCode) {
            ...
            if (broadcast_count > 0) {
                ...
                if (++log_counter % 1000 == 0) {
                    std::cout << "[WebSocketGateway] Broadcast " << batch.size() 
                              << " events to " << broadcast_count << " clients" << std::endl;
                }
            }
        }
...
void WebSocketGateway::update_stats(uint64_t messages_sent, uint64_t bytes_sent) {
